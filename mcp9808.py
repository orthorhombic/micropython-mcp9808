...
# Register pointers
REG_CONFIG = [0x01]
REG_TEMP_BOUNDARY_UPPER = [0x02]
REG_TEMP_BOUNDARY_LOWER = [0x03]
REG_TEMP_BOUNDARY_CRITICAL = [0x04]
REG_TEMP = [0x05]
...
TEMP_RESOLUTION_MIN = [0x00, 30]  # +0.5 C, refresh rate 30 ms
TEMP_RESOLUTION_LOW = [0x01, 65]  # +0.25 C, refresh rate 65 ms
TEMP_RESOLUTION_AVG = [0x02, 130]  # +0.125 C, refresh rate 130 ms
TEMP_RESOLUTION_MAX = [0x03, 250]  # +0.0625 C, refresh rate 250 ms [Default]
...
ALERT_SELECT_CRIT = [0x01]  # Ambient temp > critical


# Alert polarity
ALERT_POLARITY_ALOW = [0x00]  # Active-low, requires pull-up [Default]
ALERT_POLARITY_AHIGH = [0x01]  # Active-high


# Alert output mode
ALERT_OUTPUT_COMPARATOR = [0x00]
ALERT_OUTPUT_INTERRUPT = [0x01]
...
class MCP9808(object):
    ...
    def _send(self, buf):
        ...
        if hasattr(self._i2c, "writeto"):
            # Micropython - create bytestring for transfer
    ...
    def set_resolution(self, r):
        ...
        b = bytearray()
        b.append(REG_RESOLUTION[0])
        b.append(r[0])

        self.conversion_time = r[1]
    ...
    def get_temp(self, oneshot=True):
        ...
        One shot: wait the delay time before reading
        """
        if oneshot:
            utime.sleep_ms(self.conversion_time + 1)
        self._send(REG_TEMP)
        raw = self._recv(2)
        u = (raw[0] & 0x0F) << 4
        l = raw[1] / 16
